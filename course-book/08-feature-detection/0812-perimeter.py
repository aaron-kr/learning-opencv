...
#1
src = cv2.imread('../../img/banana.jpeg')
gray = cv2.cvtColor(src, cv2.COLOR_BGR2GRAY)
ret, bImg = cv2.threshold(gray, 220, 255, cv2.THRESH_BINARY_INV)
## bImg = cv2.erode(bImg, None)
bImg = cv2.dilate(bImg, None)
## cv2.imshow('src', src)
...
dst5 = dst2.copy()
cv2.circle(dst5, (int(x), int(y)), int(r), (0,0,255), 2)
