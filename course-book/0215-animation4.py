...
class Video(animation.FuncAnimation):
  ...
  def init(self):
    ...
    if retval:
      self.im0 = self.ax[0].imshow(cv2.cvtColor(self.frame, cv2.COLOR_BGR2RGB), aspect = 'auto') # aspect = 'equal'
      self.im1 = self.ax[1].imshow(np.zeros(self.frame.shape, self.frame.dtype), aspect = 'equal') # aspect = 'equal'
