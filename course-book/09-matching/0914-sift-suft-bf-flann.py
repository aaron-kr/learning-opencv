...
#1
src1 = cv2.imread('../../img/book1.jpg') # 'cup1.jpg'
src2 = cv2.imread('../../img/book2.jpg') # 'cup2.jpg'
...
for i, m in enumerate(matches[:3]):
  for j, n in enumerate(m):
    print('matches[{}][{}] = (queryldx:{}, trainldx:{}, distance:{})'.format(i, j, n.queryldx, n.trainldx, n.distance))
...
if len(good_matches) < 5:
  print('sorry, not enough good matches')
  exit()

#5
src1_pts = np.float32([kp1[m.queryldx].pt for m in good_matches])
src2_pts = np.float32([kp2[m.trainldx].pt for m in good_matches])
