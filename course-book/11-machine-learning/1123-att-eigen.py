...
def load_face(filename = '../data/faces.csv', test_ratio = 0.2):
  ...
  # shuffling and separate train and test data
  indices = list(range(N))
  random.seed(1) # same random sequences, so same result
...
#5: predict test_faces using y = eigenFace(face - mean) and min distance
correct_count = 0
...
for i, face in enumerate(test_faces):
  ...
  if test_labels[i] == predict_label:
    correct_count += 1
  print('test_labels = {}: predicted: {}'.format(test_labels[i], predict_label))
