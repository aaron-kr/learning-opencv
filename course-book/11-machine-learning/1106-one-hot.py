...
# ann.setTrainMethod(cv2.ml.ANN_MLP_BACKDROP)
...
trainData = cv2.ml.TrainData_create(samples = X, layout = cv2.ml.ROW_SAMPLE, responses = y_1ofN)
...
for i, k in enumerate(np.unique(y_target)):
  plt.scatter(X[y_target == k, 0], X[y_target == k, 1], c = colors[i], marker = markers[i], label = labels[k])
