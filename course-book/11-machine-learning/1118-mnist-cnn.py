...
def CNN(x):
  # weights
  W1 = tf.Variable(tf.random_normal([3,3,1,K], stddev = 0.1))
  W2 = tf.Variable(tf.random_normal([3,3,K,L], stddev = 0.1))
  W3 = tf.Variable(tf.random_normal([3,3,L,M], stddev = 0.1))
  W4 = tf.Variable(tf.random_normal([M*4*4,N], stddev = 0.1))
  W5 = tf.Variable(tf.random_normal([N,n_class], stddev = 0.1))
  ...
  P1 = tf.nn.max_pool(Y1, ksize = [1,2,2,1], strides = [1,2,2,1], padding = 'SAME')
  ...
  P2 = tf.nn.max_pool(Y2, ksize = [1,2,2,1], strides = [1,2,2,1], padding = 'SAME')
  print('P2.shape = ', P2.shape) # P2.shape = (?,7,7,64)

  Y3 = tf.nn.relu(tf.nn.conv2d(P2, W3, strides = [1,1,1,1], padding = 'SAME') + B3)
...
cross_entropy = tf.nn.softmax_cross_entropy_with_logits_v2(logits = Ylogits, labels = Y) # cross_entropy
...
with tf.Session() as sess:
  ...
  for epoch in range(100):
    ...
    for i in range(iter_per_epoch): # mini batch
      ...
      loss_batch = sess.run(loss, feed_dict = {X:x_train, Y:y_train})
      avg_loss += loss_batch
